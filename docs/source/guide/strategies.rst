...
.. literalinclude:: ../../../pydecoys/strategies/core.py
    :language: python
    :lines: 54-62

Creating a simple strategy
--------------------------
We can create a naïve randomizer that takes a target protein and returns a
fully random decoy protein of same size:
...
.. code-block:: python
    ...
    def random(sequence: str) -> str:
        length = len(sequence)
        return RAND.choices(AMINOACIDS, k=length)

This will work perfectly already, and for most usecases it might be enough.
...
It also isn't interfaceable with Biopython, which means it might break or at
least return a `str` when it should return `Seq` or `MutableSeq`. Fixing this
is easy:
...
.. code-block:: python
    ...
    def random(sequence: SeqLike) -> SeqLike:
        ...
        return seq_cast(sequence, RAND.choices(AMINOACIDS, k=length))
...
solve that, PyDecoys introduces two alias that are `str` at runtime, and don't
require Biopython:
...
dependency! Better yet, if it does receive Biopython objects, it'll handle them
correctly.
...
would need some form of gathering context from the database prior to decoy
generation and saving it as state.

PyDecoys has a specific class for that:
:py:class:`strategies.ContextfulGenerator`. When a function that takes a set of
targets receives a `ContextfulGenerator` as strategy, it passes all the set
database to it beforehand.

.. literalinclude:: ../../../pydecoys/strategies/core.py
    :language: python
    :lines: 88-105

It's a :py:class:`strategies.DecoyGenerator`, so we'll need to override the
...
All set! Now it's ready to be used or registed through :py:func:`register`!
...
strategies. Look at the ``__init__`` for their base class:

.. literalinclude:: ../../../pydecoys/strategies/core.py
    :language: python
    :lines: 108,130-136

Setting new enzymes is an easy instantiation. Let's set high-specificity
...
--------------------------
You might want to set your own enzyme-specific strategy. Luckily, you already
saw the ABC for that: :py:class:`strategies.EnzymeSpecificGenerator`. This
class sets up the ``__init__`` method we used earlier to add the new enzyme. It
also sets ``cut``, ``nocut``, ``sense`` and ``keep_term`` get-only properties!
